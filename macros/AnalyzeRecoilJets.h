...
// ===========================================================================
//   PP DATA ONLY:   isPPdataOnly=true,  isSimAndDataPP=false,  isSimAndDataAUAU=false
//   SIM ONLY:       isPPdataOnly=false, isSimAndDataPP=false,  isSimAndDataAUAU=false  + one SIM toggle
//   SIM + PP DATA:  isPPdataOnly=false, isSimAndDataPP=true,   isSimAndDataAUAU=false  + one PP-SIM toggle
//   SIM + AuAu:     isPPdataOnly=false, isSimAndDataPP=false,  isSimAndDataAUAU=true   + one embedded-SIM toggle
...
inline bool DO_inclusiveXJcomparisons_ONLY        = true;
...
{
    return kInputBase + "/simEmbedded/RecoilJets_" + sampleTag + "_ALL_" + CfgTagWithUE() + ".root";
}
...
inline string OutputCombinedSimOnlyEMBEDDED(const string& comboLabel)
{
    return kOutputBase + "/combinedSimOnlyEMBEDDED/" + CfgTagWithUE() + "/" + comboLabel;
}
inline string OutputCombinedSimOnlyEMBEDDED(const string& cfgTag, const string& comboLabel)
{
    return kOutputBase + "/combinedSimOnlyEMBEDDED/" + cfgTag + "/" + comboLabel;
}
...
inline void CollectDiscoveryObjectNamesRecursive(TDirectory* dir, const string& prefix, vector<string>& out)
{
    if (!dir) return;
    ...
    while (TKey* key = (TKey*)next())
    {
        ...
        if (cls == "TDirectoryFile" || cls == "TDirectory")
        {
            ...
            if (sub) CollectDiscoveryObjectNamesRecursive(sub, full + "/", out);
            continue;
        }
...
inline const vector<string>& DiscoveryInputPaths()
{
    static const vector<string> paths = []()
    {
        ...
        if (isAuAuOnly || isSimAndDataAUAU || isPPdataAndAUAU) add(InputAuAu());
        if (isPPdataOnly || isSimAndDataPP || isPPdataAndAUAU) add(InputPP(isRun25pp));
        
        if (isPhotonJet5 || bothPhoton5and10sim || bothPhoton5and20sim || allPhoton5and10and20sim)
            add(InputSim("photonjet5"));
        if (isPhotonJet10 || bothPhoton5and10sim || bothPhoton10and20sim || allPhoton5and10and20sim)
            add(InputSim("photonjet10"));
        if (isPhotonJet20 || bothPhoton5and20sim || bothPhoton10and20sim || allPhoton5and10and20sim)
...
inline bool ExtractCentRangeFromName(const string& name, int& lo, int& hi)
{
    const string tag = "_cent_";
    const size_t pos = name.find(tag);
    if (pos == string::npos) return false;
    
    const size_t loStart = pos + tag.size();
    size_t loEnd = loStart;
    while (loEnd < name.size() && name[loEnd] >= '0' && name[loEnd] <= '9') ++loEnd;
    if (loEnd == loStart || loEnd >= name.size() || name[loEnd] != '_') return false;
    
    const size_t hiStart = loEnd + 1;
    size_t hiEnd = hiStart;
    while (hiEnd < name.size() && name[hiEnd] >= '0' && name[hiEnd] <= '9') ++hiEnd;
    if (hiEnd == hiStart) return false;
    
    try
    {
        lo = std::stoi(name.substr(loStart, loEnd - loStart));
        hi = std::stoi(name.substr(hiStart, hiEnd - hiStart));
    }
    catch (...)
    {
        return false;
...
inline bool ExtractRKeyFromName(const string& name, string& rKey)
{
    size_t pos = name.find("_r");
    while (pos != string::npos)
    {
        const size_t keyStart = pos + 2;
        size_t keyEnd = keyStart;
        while (keyEnd < name.size() && name[keyEnd] >= '0' && name[keyEnd] <= '9') ++keyEnd;
        
        if (keyEnd > keyStart)
        {
            if (keyEnd == name.size() || name[keyEnd] == '_' || name[keyEnd] == '/')
            {
                rKey = "r" + name.substr(keyStart, keyEnd - keyStart);
                return true;
            }
        }
        
        pos = name.find("_r", pos + 2);
    }
    
    return false;
}

inline vector<double> DiscoverCentralityEdgesFromFile(const string& path)
{
    vector<double> out;
    if (path.empty() || gSystem->AccessPathName(path.c_str())) return out;
    ...
    vector<string> names;
    CollectDiscoveryObjectNamesRecursive(f, "", names);
    ...
    for (const auto& name : names)
    {
        int lo = 0;
        int hi = 0;
        if (!ExtractCentRangeFromName(name, lo, hi)) continue;
        edges.insert(lo);
        edges.insert(hi);
    }
    ...
    for (const int edge : edges) out.push_back((double)edge);
    return out;
}

inline vector<string> DiscoverRKeysFromFile(const string& path)
{
    vector<string> out;
    if (path.empty() || gSystem->AccessPathName(path.c_str())) return out;
    ...
    {
        string rKey;
        if (!ExtractRKeyFromName(name, rKey)) continue;
        keys.insert(rKey);
    }
...
inline vector<double> DiscoverCentralityEdgesFromInputFiles()
{
    for (const auto& path : DiscoveryInputPaths())
    {
        vector<double> edges = DiscoverCentralityEdgesFromFile(path);
...
inline vector<string> DiscoverRKeysFromInputFiles()
{
    for (const auto& path : DiscoveryInputPaths())
    {
        vector<string> keys = DiscoverRKeysFromFile(path);
...
inline const BinningCfg& Binning()
{
    ...
    vector<double> discoveredCentEdges = DiscoverCentralityEdgesFromInputFiles();
    if (discoveredCentEdges.size() >= 2)
        cfg.centrality_edges = discoveredCentEdges;
...
inline bool ValidateRunConfig(string* errMsg = nullptr)
{
    ...
    {
        if (errMsg) *errMsg = "isPPdataOnly is mutually exclusive with isSimAndDataPP and isSimAndDataAUAU. Choose only one.";
        return false;
    }
    
    if (isSimAndDataPP && isSimAndDataAUAU)
    {
        if (errMsg) *errMsg = "isSimAndDataPP and isSimAndDataAUAU cannot both be true. Choose only one combined run mode.";
        return false;
    }
    
    if (isAuAuOnly && (isPPdataOnly || isSimAndDataPP))
    {
        if (errMsg) *errMsg = "isAuAuOnly=true is mutually exclusive with isPPdataOnly and isSimAndDataPP. It may be combined with isSimAndDataAUAU.";
        return false;
    }
    
    const SimSample ss = CurrentSimSample();
    
    // Pure AuAu-only run: SIM sample toggles must be OFF.
    ...
    // Any non-PP-only run includes SIM (either SIM-only or SIM+DATA).
    if (ss == SimSample::kNone)
    {
        if (errMsg)
        {
    ...
    // SIM+PP mode contract.
    if (isSimAndDataPP &&
        (IsEmbeddedSimSample(ss) ||
          ...
          ss != SimSample::kSimJet5)))
    {
        ...
        {
            *errMsg =
            "SIM+DATA PP (isSimAndDataPP=true) requires a merged photonJet10+20 or photonJet5+10+20 SIM sample, "
            ...
            "(all embedded SIM toggles false).";
        }
        return false;
    }
...
inline string RunModeLabel(RunMode m)
{
    switch (m)
    {
        ...
        case RunMode::kSimAndDataAUAU:return "SIM_AND_DATA_AUAU";
        default:                      return "INVALID";
    }
}
