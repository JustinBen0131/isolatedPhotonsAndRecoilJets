...
std::vector<Row> read_rows(const std::string& csv_path)
{
  ...
  while (std::getline(in, line))
  {
    ...
    r.current = to_double(vals, col, "the94_canonical_value");
    r.current_err = to_double(vals, col, "the94_canonical_error");
    ...
    r.current_over_fit = to_double(vals, col, "the94_canonical_over_fit");
    r.current_over_fit_err = to_double(vals, col, "the94_canonical_over_fit_error");
    r.current_over_ppg12 = to_double(vals, col, "the94_canonical_over_ppg12_reference");
...
TGraphErrors* make_fit_graph(const std::vector<Row>& rows)
{
  ...
  auto* g = new TGraphErrors();
  g->SetName("g_ppg12_fit_canonical_jet8");
...
void style_top(TGraphErrors* g, int color, int marker)
{
  ...
  g->SetMarkerSize(marker == 24 ? 0.85 : 0.72);
  g->SetLineWidth(1);
...
void style_ratio(TGraphErrors* g, int marker)
{
  ...
  g->SetMarkerSize(marker == 24 ? 0.75 : 0.63);
  g->SetLineWidth(1);
  if (marker == 24) g->SetFillStyle(0);
}
}  // namespace

void MakeTHE94JetFig6CanonicalJet8PolicyRootStyle(const char* csv_override = "",
                                                  const char* png_override = "",
                                                  const char* manifest_override = "")
{
  const std::string base =
      "/Users/patsfan753/Desktop/ThesisAnalysis/dataOutput/ppg12Parity/"
      "the94_ppg12_inclusivejet_fig6_fixed_20260702_204032/canonical_jet8_policy_overlay";
  const std::string default_csv = base + "/inclusivejet_fig6_canonical_jet8_policy_points.csv";
  ...
  const std::string png_path = std::string(png_override).empty()
                                   ? base + "/inclusivejet_fig6_canonical_jet8_policy_rootstyle.png"
                                   : std::string(png_override);
  const std::string manifest_path = std::string(manifest_override).empty()
                                        ? base + "/inclusivejet_fig6_canonical_jet8_policy_rootstyle_manifest.json"
  ...
  for (const auto& s : samples)
  {
    ...
    g_current_top[s] = make_graph(rs, "g_the94_canonical_top_" + s, &Row::current, &Row::current_err);
    g_ppg12_top[s] = make_graph(rs, "g_ppg12_historical_top_" + s, &Row::ppg12, &Row::ppg12_err);
    g_current_ratio[s] = make_graph(rs, "g_the94_canonical_ratio_" + s, &Row::current_over_fit, &Row::current_over_fit_err);
    g_ppg12_ratio[s] = make_graph(rs, "g_ppg12_historical_ratio_" + s, &Row::ppg12_over_fit, &Row::ppg12_over_fit_err);
  ...
  TCanvas* c4 = new TCanvas("can_the94_canonical_jet8_policy", "", 800, 889);
  ...
  frame_et_rec->GetYaxis()->SetRangeUser(5e3, 6.0e12);
  ...
  myText(0.47, 0.88, 1, strleg1.c_str(), 0.048);
  myText(0.47, 0.825, 1, strleg2.c_str(), 0.048);
  myText(0.47, 0.77, 1, strMC.c_str(), 0.048);

  myMarkerLineText(0.69, 0.69, 1, colors.at("jet8"), 20, colors.at("jet8"), 1, "jet8", 0.043, true);
  myMarkerLineText(0.69, 0.64, 1, colors.at("jet12"), 20, colors.at("jet12"), 1, "jet12", 0.043, true);
  myMarkerLineText(0.69, 0.59, 1, colors.at("jet20"), 20, colors.at("jet20"), 1, "jet20", 0.043, true);
  myMarkerLineText(0.69, 0.54, 1, colors.at("jet30"), 20, colors.at("jet30"), 1, "jet30", 0.043, true);
  myMarkerLineText(0.69, 0.49, 1, colors.at("jet40"), 20, colors.at("jet40"), 1, "jet40", 0.043, true);

  myText(0.18, 0.31, 1, "source", 0.036);
  myMarkerLineText(0.18, 0.26, 1, kBlack, 24, kBlack, 1, "PPG12 ref.", 0.038, true);
  myMarkerLineText(0.18, 0.21, 1, kBlack, 20, kBlack, 1, "THE-94 canonical", 0.038, true);
  myMarkerLineText(0.18, 0.16, 0, kRed - 4, 0, kRed - 4, 1, "PPG12 fit", 0.038, true);
  myText(0.18, 0.09, kBlack, "jet8: no hidden 0.427 factor", 0.030);

  TPad* p2 = static_cast<TPad*>(c4->cd(2));
  p2->SetPad(0, 0, 1, 0.4);
  p2->SetTopMargin(0.023);
  p2->SetLeftMargin(0.13);
  p2->SetBottomMargin(0.27);
  ...
  frame_et_truth->GetYaxis()->SetRangeUser(0.55, 2.90);
  frame_et_truth->GetXaxis()->SetTitleSize(0.095);
  frame_et_truth->GetXaxis()->SetLabelSize(0.070);
  frame_et_truth->GetYaxis()->SetTitleSize(0.078);
  frame_et_truth->GetYaxis()->SetLabelSize(0.060);
  frame_et_truth->GetYaxis()->SetTitleOffset(0.72);
  frame_et_truth->GetXaxis()->SetTitleOffset(1.02);
  frame_et_truth->GetXaxis()->SetNdivisions(505);
  frame_et_truth->GetYaxis()->SetNdivisions(508);
  ...
  manifest << "{\n";
  manifest << "  \"status\": \"ok_canonical_jet8_policy_rootstyle\",\n";
  manifest << "  \"png\": \"" << png_path << "\",\n";
  manifest << "  \"points_csv\": \"" << csv_path << "\",\n";
  manifest << "  \"interpretation\": \"canonical RecoilJets jet8 policy comparison; not PPG12-verbatim jet8 parity\",\n";
  manifest << "  \"jet8_current_display_scale_factor\": 1.0,\n";
  manifest << "  \"top_y_range\": [5000.0, 6000000000000.0],\n";
  manifest << "  \"bottom_y_range\": [0.55, 2.90],\n";
  manifest << "  \"note\": \"Top panel follows IAN organization but y max is extended so the no-scale canonical jet8 points are visible; lower panel is expanded to show jet8-to-jet12 continuity.\"\n";
  manifest << "}\n";
  manifest.close();
}
