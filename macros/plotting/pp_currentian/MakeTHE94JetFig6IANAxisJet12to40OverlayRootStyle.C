...
struct Row
{
  ...
  double the94 = 0;
  double the94_err = 0;
  double the94_over_fit = 0;
  double the94_over_fit_err = 0;
  double the94_over_ppg12 = 0;
  bool draw_the94 = false;
};
...
std::vector<Row> read_rows(const std::string& csv_path)
{
  ...
  while (std::getline(in, line))
  {
    ...
    r.ppg12 = to_double(vals, col, "ppg12_ian_value");
    r.ppg12_err = to_double(vals, col, "ppg12_ian_error");
    r.fit = to_double(vals, col, "ppg12_fit_value");
    r.ppg12_over_fit = to_double(vals, col, "ppg12_ian_over_fit");
    r.ppg12_over_fit_err = to_double(vals, col, "ppg12_ian_over_fit_error");
    r.the94 = to_double(vals, col, "the94_display_value");
    r.the94_err = to_double(vals, col, "the94_display_error");
    r.the94_over_fit = to_double(vals, col, "the94_display_over_fit");
    r.the94_over_fit_err = to_double(vals, col, "the94_display_over_fit_error");
    r.the94_over_ppg12 = to_double(vals, col, "the94_display_over_ppg12_ian");
    r.draw_the94 = to_double(vals, col, "draw_the94") > 0.5;
    if (r.x <= 50.0) rows.push_back(r);
  }
  return rows;
}

std::vector<Row> by_sample(const std::vector<Row>& rows, const std::string& sample, bool require_the94 = false)
{
  std::vector<Row> out;
  for (const auto& r : rows)
  {
    if (r.sample != sample) continue;
    if (require_the94 && !r.draw_the94) continue;
    out.push_back(r);
  }
...
TGraphErrors* make_fit_graph(const std::vector<Row>& rows)
{
  ...
  auto* g = new TGraphErrors();
  g->SetName("g_ppg12_jet_fit_ian_axis_the94_jet12to40");
...
void style_ppg12_top(TGraphErrors* g, int color)
{
  g->SetMarkerStyle(20);
...
void style_the94_top(TGraphErrors* g, int color)
{
  g->SetMarkerStyle(24);
  g->SetMarkerColor(color);
  g->SetLineColor(color);
...
void style_ppg12_ratio(TGraphErrors* g)
{
  g->SetMarkerStyle(20);
...
void style_the94_ratio(TGraphErrors* g)
{
  g->SetMarkerStyle(24);
  g->SetMarkerColor(kBlack);
  g->SetLineColor(kBlack);
...
void MakeTHE94JetFig6IANAxisJet12to40OverlayRootStyle(const char* csv_override = "",
                                                       const char* png_override = "",
                                                       const char* manifest_override = "")
{
  const std::string base =
      "/Users/patsfan753/Desktop/ThesisAnalysis/dataOutput/ppg12Parity/"
      "the94_ppg12_inclusivejet_fig6_fixed_20260702_204032/ian_axis_jet12to40_overlay";
  const std::string input_csv =
      base + "/inclusivejet_fig6_ian_axis_ppg12_jet8to40_the94_jet12to40_points.csv";
  ...
  const std::string png_path = std::string(png_override).empty()
                                   ? base + "/inclusivejet_fig6_ian_axis_ppg12_jet8to40_the94_jet12to40_overlay.png"
                                   : std::string(png_override);
  const std::string manifest_path = std::string(manifest_override).empty()
                                        ? base + "/inclusivejet_fig6_ian_axis_ppg12_jet8to40_the94_jet12to40_overlay_manifest.json"
  ...
  std::vector<std::string> the94_samples;
  for (const auto& s : samples)
  {
    auto rs = by_sample(rows, s, true);
    if (!rs.empty()) the94_samples.push_back(s);
  }
  ...
  std::map<std::string, TGraphErrors*> g_the94_top;
  std::map<std::string, TGraphErrors*> g_ppg12_ratio;
  std::map<std::string, TGraphErrors*> g_the94_ratio;
  ...
  for (const auto& s : samples)
  {
    ...
    style_ppg12_top(g_ppg12_top[s], colors.at(s));
    style_ppg12_ratio(g_ppg12_ratio[s]);
  }

  for (const auto& s : the94_samples)
  {
    auto rs = by_sample(rows, s, true);
    g_the94_top[s] = make_graph(rs, ("g_the94_ian_axis_top_" + s).c_str(), &Row::the94, &Row::the94_err);
    g_the94_ratio[s] = make_graph(rs, ("g_the94_ian_axis_ratio_" + s).c_str(), &Row::the94_over_fit, &Row::the94_over_fit_err);
    style_the94_top(g_the94_top[s], colors.at(s));
    style_the94_ratio(g_the94_ratio[s]);
  }
  ...
  TCanvas* c4 = new TCanvas("can_the94_ppg12_jet_ian_axis_jet12to40", "", 800, 889);
  ...
  for (const auto& s : the94_samples) g_the94_top[s]->Draw("PE same");
  g_fit->Draw("L same");
  ...
  myText(0.20, 0.31, 1, "sample", 0.034);
  myMarkerLineText(0.20, 0.26, 1, colors.at("jet8"), 20, colors.at("jet8"), 1, "jet8", 0.035, true);
  myMarkerLineText(0.20, 0.21, 1, colors.at("jet12"), 20, colors.at("jet12"), 1, "jet12", 0.035, true);
  myMarkerLineText(0.20, 0.16, 1, colors.at("jet20"), 20, colors.at("jet20"), 1, "jet20", 0.035, true);
  myMarkerLineText(0.20, 0.11, 1, colors.at("jet30"), 20, colors.at("jet30"), 1, "jet30", 0.035, true);
  myMarkerLineText(0.20, 0.06, 1, colors.at("jet40"), 20, colors.at("jet40"), 1, "jet40", 0.035, true);

  myText(0.42, 0.31, 1, "source", 0.034);
  myMarkerLineText(0.42, 0.26, 1, kBlack, 20, kBlack, 1, "PPG12 SDCC", 0.035, true);
  myMarkerLineText(0.42, 0.21, 1, kBlack, 24, kBlack, 1, "Current output", 0.035, true);
  ...
  for (const auto& s : the94_samples) g_the94_ratio[s]->Draw("PE same");
  lineone->Draw("L");
  ...
  out << "{\n";
  out << "  \"status\": \"ok_ian_axis_ppg12_jet8to40_the94_overlay\",\n";
  ...
  out << "  \"style_contract\": \"PPG12 IAN Fig.6 two-pad canvas, 9-50 GeV x range, 5e3-2e12 log-y spectrum range, 0.85-1.15 MC/Fit range, and PPG12 fit values from the IAN/source-scope CSV.\",\n";
  out << "  \"legend_ppg12_label\": \"PPG12 SDCC\",\n";
  out << "  \"legend_current_label\": \"Current output\",\n";
  out << "  \"ppg12_display\": \"jet8/jet12/jet20/jet30/jet40 all drawn from the PPG12 IAN/source-scope point table\",\n";
  out << "  \"the94_display\": \"drawn for samples marked draw_the94=1 in the points CSV\",\n";
  out << "  \"interpretation\": \"IAN-axis source-scope display comparison; current jet8 may be included with an explicit display alignment factor from the points CSV; not nominal no-scale jet8 parity\",\n";
  out << "  \"displayed_rows\": " << rows.size() << "\n";
  out << "}\n";
  out.close();
}
