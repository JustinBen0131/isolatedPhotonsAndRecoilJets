    ...
    void RunRooUnfoldPipeline_SimAndDataPP(Dataset& dsData, Dataset& dsSim)
    {
                    ...
                    for (int i = 0; i < (int)analysisRecoBins.size(); ++i)
                    {
                        ...
                        if (hXJRecoData)
                        {
                            ...
                            
                            {
                                ...
                                static TFile* s_ppFallbackFile = nullptr;
                                static TDirectory* s_ppFallbackTop = nullptr;
                                
                                const std::string ppFallbackPath = InputPP(isRun25pp);
                                if (!s_ppFallbackFile || s_ppFallbackPath != ppFallbackPath)
                                {
                                    if (s_ppFallbackFile)
                                    {
                                        s_ppFallbackFile->Close();
                                        delete s_ppFallbackFile;
                                        s_ppFallbackFile = nullptr;
                                    }
                                    s_ppFallbackTop = nullptr;
                                    
                                    s_ppFallbackFile = TFile::Open(ppFallbackPath.c_str(), "READ");
                                    s_ppFallbackPath = ppFallbackPath;
                                    
                                    if (s_ppFallbackFile && !s_ppFallbackFile->IsZombie())
                                    {
                                        s_ppFallbackTop = s_ppFallbackFile->GetDirectory(kTriggerPP.c_str());
                                        if (!s_ppFallbackTop) s_ppFallbackTop = s_ppFallbackFile;
                                    }
                                }
                                ...
                                if (hXJRecoPP)
                                {
                                    TH1* hAuShape = CloneTH1(
                                                             hXJRecoData,
                                                             TString::Format("h_xJRecoAuShape_%s_%s", rKey.c_str(), b.folder.c_str()).Data()
                                                             );
                                    TH1* hPPShape = CloneTH1(
                                                             hXJRecoPP,
                                                             TString::Format("h_xJRecoPPShape_%s_%s", rKey.c_str(), b.folder.c_str()).Data()
                                                             );
                                    
                                    if (hAuShape && hPPShape)
                                    {
                                        hAuShape->SetDirectory(nullptr);
                                        hPPShape->SetDirectory(nullptr);
                                        EnsureSumw2(hAuShape);
                                        EnsureSumw2(hPPShape);
                                        
                                        NormalizeToUnitArea(hAuShape);
                                        NormalizeToUnitArea(hPPShape);
                                        
                                        hAuShape->SetTitle("");
                                        hAuShape->SetLineColor(kBlack);
                                        hAuShape->SetMarkerColor(kBlack);
                                        hAuShape->SetMarkerStyle(20);
                                        hAuShape->SetMarkerSize(1.0);
                                        hAuShape->SetLineWidth(2);
                                        hAuShape->GetXaxis()->SetTitle("x_{J#gamma}");
                                        hAuShape->GetYaxis()->SetTitle("Fraction of entries");
                                        
                                        hPPShape->SetLineColor(kRed + 1);
                                        hPPShape->SetMarkerColor(kRed + 1);
                                        hPPShape->SetMarkerStyle(24);
                                        hPPShape->SetMarkerSize(1.0);
                                        hPPShape->SetLineWidth(2);
                                        
                                        const double maxv = std::max(hAuShape->GetMaximum(), hPPShape->GetMaximum());
                                        
                                        TCanvas cPP(
                                                    TString::Format("c_xJRecoDataInputAuAuVsPP_%s_%s", rKey.c_str(), b.folder.c_str()).Data(),
                                                    "c_xJRecoDataInputAuAuVsPP", 900, 700
                                                    );
                                        ApplyCanvasMargins1D(cPP);
                                        
                                        hAuShape->SetMaximum((maxv > 0.0) ? (1.35 * maxv) : 1.0);
                                        hAuShape->Draw("E1");
                                        hPPShape->Draw("E1 same");
                                        ...
                                        leg.AddEntry(hAuShape, TString::Format("AuAu reco input (%s)", currentCentLabel.c_str()).Data(), "lep");
                                        leg.AddEntry(hPPShape, "pp reco input", "lep");
                                        leg.Draw();
                                        
                                        vector<string> lines = DefaultHeaderLines(dsData);
                                        lines.push_back("Normalized reco x_{J#gamma} input overlay: AuAu vs pp");
                                        lines.push_back(TString::Format("R = %.1f, p_{T}^{#gamma}: %d-%d GeV", R, b.lo, b.hi).Data());
                                        lines.push_back("Both histograms normalized to unit area");
                                        DrawLatexLines(0.14, 0.92, lines, 0.032, 0.043);
                                        
                                        SaveCanvas(cPP, JoinPath(ptOut, "xJ_recoData_input_overlay_auau_vs_pp_unitArea.png"));
                                    }
                                    
                                    if (hAuShape) delete hAuShape;
                                    if (hPPShape) delete hPPShape;
                                    delete hXJRecoPP;
                                }
                            }
                        }
        ...
        // ----------------------------------------------------------------------
        // (B.2) all-radii overlay tables saved to <outBase>/withPerPhotonNormalization/
        //       and <outBase>/withoutPerPhotonNormalization/
        //       All overlay outputs are normalized to unit area.
        // ----------------------------------------------------------------------
        {
            ...
            if (any)
            {
                ...
                std::map<std::string, TFile*> rawOverlayFiles;
                static long long sOverlayCloneSerial = 0;
                
                auto OpenRawOverlayRoot = [&](const std::string& rKey)->TFile*
                {
                    auto it = rawOverlayFiles.find(rKey);
                    if (it != rawOverlayFiles.end()) return it->second;
                    
                    const std::string fpath = JoinPath(JoinPath(outBase, rKey), "rooUnfold_outputs.root");
                    ...
                    rawOverlayFiles[rKey] = f;
                    return f;
                };
                
                auto NormalizeOverlayToUnitArea = [&](TH1* h)->void
                {
                    if (!h) return;
                    ...
                    if (!(std::isfinite(integ) && integ > 0.0))
                    {
                        integ = h->Integral(1, nb);
                    }
                ...
                auto StyleRadiiOverlayHist = [&](TH1* h, const std::string& rKey)->void
                {
                    if (!h) return;
                    ...
                    h->GetXaxis()->SetRangeUser(0.0, 2.0);
                    h->GetXaxis()->SetTitle("x_{J}");
                    h->GetYaxis()->SetTitle("Fraction of entries");
                ...
                auto LoadWithPerPhotonHist =
                [&](const std::string& rKey,
                    int i,
                    std::vector<TH1*>& keepAlive)->TH1*
                {
                    auto it = perPhoHistsByRKey.find(rKey);
                    if (it == perPhoHistsByRKey.end()) return nullptr;
                    if (i < 0 || i >= (int)it->second.size()) return nullptr;
                    if (!it->second[i]) return nullptr;
                    
                    TH1* h = (TH1*)it->second[i]->Clone(
                                                        TString::Format("h_withPerPho_%s_%d_%lld", rKey.c_str(), i, ++sOverlayCloneSerial).Data()
                                                        );
                    if (!h) return nullptr;
                    
                    h->SetDirectory(nullptr);
                    EnsureSumw2(h);
                    NormalizeOverlayToUnitArea(h);
                    StyleRadiiOverlayHist(h, rKey);
                    keepAlive.push_back(h);
                    return h;
                };
                
                auto LoadWithoutPerPhotonHist =
                [&](const std::string& rKey,
                    int i,
                    std::vector<TH1*>& keepAlive)->TH1*
                {
                    if (i < 0 || i >= nPtAll) return nullptr;
                    
                    TFile* f = OpenRawOverlayRoot(rKey);
                    if (!f) return nullptr;
                    
                    TH2* h2 = dynamic_cast<TH2*>(f->Get("h2_truthUnfold_pTgamma_xJ"));
                    if (!h2) return nullptr;
                    
                    const PtBin& b = analysisRecoBins[i];
                    
                    TH1D* hProj = ProjectTH2YForPtWindow(
                                                         h2,
                                                         b,
                                                         TString::Format("h_withoutPerPhoProj_%s_%s_%lld", rKey.c_str(), b.folder.c_str(), ++sOverlayCloneSerial).Data(),
                                                         "e"
                                                         );
                    ...
                    EnsureSumw2(hProj);
                    NormalizeOverlayToUnitArea(hProj);
                    StyleRadiiOverlayHist(hProj, rKey);
                    keepAlive.push_back(hProj);
                    return hProj;
                };
                
                auto LoadJetEffPerPhotonHist =
                [&](const std::string& rKey,
                    int i,
                    std::vector<TH1*>& keepAlive)->TH1*
                {
                    auto it = perPhoHistsJetEffCorrByRKey.find(rKey);
                    if (it == perPhoHistsJetEffCorrByRKey.end()) return nullptr;
                    if (i < 0 || i >= (int)it->second.size()) return nullptr;
                    if (!it->second[i]) return nullptr;
                    
                    TH1* h = (TH1*)it->second[i]->Clone(
                                                        TString::Format("h_withPerPhoJetEff_%s_%d_%lld", rKey.c_str(), i, ++sOverlayCloneSerial).Data()
                                                        );
                    if (!h) return nullptr;
                ...
                auto DrawRadiiOverlayPad =
                [&](int i,
                    bool usePerPhoton,
                    bool useJetEff,
                    std::vector<TH1*>& keepAlive)->void
                {
                    if (i < 0 || i >= nPtAll)
                    {
                        TH1F* frame = new TH1F(
                                               TString::Format("frameMissing_%lld", ++sOverlayCloneSerial).Data(),
                                               "", 1, 0.0, 2.0
                                               );
                        ...
                        frame->SetTitle("");
                        frame->GetXaxis()->SetTitle("x_{J}");
                        frame->GetYaxis()->SetTitle("Fraction of entries");
                        frame->Draw("axis");
                        keepAlive.push_back(frame);
                    ...
                    if (useJetEff)
                    {
                        h02 = LoadJetEffPerPhotonHist("r02", i, keepAlive);
                        h04 = LoadJetEffPerPhotonHist("r04", i, keepAlive);
                        h06 = LoadJetEffPerPhotonHist("r06", i, keepAlive);
                    }
                    else if (usePerPhoton)
                    {
                        h02 = LoadWithPerPhotonHist("r02", i, keepAlive);
                        h04 = LoadWithPerPhotonHist("r04", i, keepAlive);
                        h06 = LoadWithPerPhotonHist("r06", i, keepAlive);
                    }
                    else
                    {
                        h02 = LoadWithoutPerPhotonHist("r02", i, keepAlive);
                        h04 = LoadWithoutPerPhotonHist("r04", i, keepAlive);
                        h06 = LoadWithoutPerPhotonHist("r06", i, keepAlive);
                    }
                    ...
                    TH1F* frame = new TH1F(
                                           TString::Format("frame_%s_%d_%lld",
                                                           (useJetEff ? "jetEff" : (usePerPhoton ? "withPerPho" : "withoutPerPho")),
                                                           i,
                                                           ++sOverlayCloneSerial).Data(),
                                           "", 1, 0.0, 2.0
                                           );
                    frame->SetDirectory(nullptr);
                    frame->SetMinimum(0.0);
                    ...
                    if (h02) h02->Draw("E1 X0 same");
                    if (h04) h04->Draw("E1 X0 same");
                    if (h06) h06->Draw("E1 X0 same");
                    
                    {
                        ...
                        {
                            tx.DrawLatex(0.52, 0.955,
                                         TString::Format("Unit-area jet-eff corrected per-photon x_{J#gamma}, p_{T}^{#gamma} %d-%d GeV",
                                                         b.lo, b.hi).Data());
                        }
                        else if (usePerPhoton)
                        {
                            tx.DrawLatex(0.52, 0.955,
                                         TString::Format("Unit-area per-photon x_{J#gamma}, p_{T}^{#gamma} %d-%d GeV",
                                                         b.lo, b.hi).Data());
                        }
                        else
                        {
                            tx.DrawLatex(0.52, 0.955,
                                         TString::Format("Unit-area x_{J#gamma} without per-photon normalization, p_{T}^{#gamma} %d-%d GeV",
                                                         b.lo, b.hi).Data());
                        }
                    }
                    ...
                    {
                        TLegend leg(0.62, 0.50, 0.95, 0.70);
                        leg.SetBorderSize(0);
                        leg.SetFillStyle(0);
                        leg.SetTextFont(42);
                        leg.SetTextSize(0.034);
                        
                        if (h02) leg.AddEntry(h02, "R = 0.2", "pe");
                        if (h04) leg.AddEntry(h04, "R = 0.4", "pe");
                        if (h06) leg.AddEntry(h06, "R = 0.6", "pe");
                        
                        leg.DrawClone();
                    }
                };
                
                auto WriteRadiiOverlayBundle =
                [&](const std::string& outDir,
                    const std::string& tableName,
                    const std::string& singleStem,
                    bool usePerPhoton,
                    bool useJetEff)->void
                {
                    EnsureDir(outDir);
                    ...
                    {
                        TCanvas c(
                                  TString::Format("c_tbl_overlay_%s_%lld",
                                                  (useJetEff ? "jetEff" : (usePerPhoton ? "withPerPho" : "withoutPerPho")),
                                                  ++sOverlayCloneSerial).Data(),
                                  "c_tbl_overlay", 1800, 1300
                                  );
                        ...
                        for (int ipad = 0; ipad < nPads; ++ipad)
                        {
                            ...
                            DrawRadiiOverlayPad(ipad, usePerPhoton, useJetEff, keepAlive);
                        }
                        
                        SaveCanvas(c, JoinPath(outDir, tableName));
                    ...
                    for (int i = 0; i < nPtAll; ++i)
                    {
                        ...
                        TCanvas cSingle(
                                        TString::Format("c_single_overlay_%s_%s_%lld",
                                                        (useJetEff ? "jetEff" : (usePerPhoton ? "withPerPho" : "withoutPerPho")),
                                                        b.folder.c_str(),
                                                        ++sOverlayCloneSerial).Data(),
                                        "c_single_overlay", 900, 700
                                        );
                        ApplyCanvasMargins1D(cSingle);
                        
                        std::vector<TH1*> keepAlive;
                        DrawRadiiOverlayPad(i, usePerPhoton, useJetEff, keepAlive);
                        
                        SaveCanvas(
                                   cSingle,
                                   JoinPath(
                                            outDir,
                                            TString::Format("%s_%s.png", singleStem.c_str(), b.folder.c_str()).Data()
                                            )
                                   );
                ...
                const std::string withPerPhotonBase    = JoinPath(outBase, "withPerPhotonNormalization");
                const std::string withoutPerPhotonBase = JoinPath(outBase, "withoutPerPhotonNormalization");
                
                WriteRadiiOverlayBundle(
                                        JoinPath(withPerPhotonBase, "DNDXJ_perRADII"),
                                        "table3x3_unfolded_perPhoton_dNdXJ_overlay_radii.png",
                                        "unfolded_perPhoton_dNdXJ_overlay_radii",
                                        true,
                                        false
                                        );
                
                WriteRadiiOverlayBundle(
                                        JoinPath(withoutPerPhotonBase, "DNDXJ_perRADII"),
                                        "table3x3_unfolded_noPerPhotonNormalization_dNdXJ_overlay_radii.png",
                                        "unfolded_noPerPhotonNormalization_dNdXJ_overlay_radii",
                                        false,
                                        false
                                        );
                ...
                {
                    WriteRadiiOverlayBundle(
                                            JoinPath(withPerPhotonBase, "jetEffRadiiOverlayDNDXJ"),
                                            "table3x3_unfolded_perPhoton_dNdXJ_overlay_radii_jetEffCorr.png",
                                            "unfolded_perPhoton_dNdXJ_overlay_radii_jetEffCorr",
                                            true,
                                            true
                                            );
                }
                
                for (auto& kv : rawOverlayFiles)
                {
                    if (kv.second)
                    {
                        kv.second->Close();
