    ...
    void RunRooUnfoldPipeline_SimAndDataPP(Dataset& dsData, Dataset& dsSim)
{
        ...
        for (const auto& rKey : kRKeys)
        {
            ...
            if (h2UnfoldTruth)
            {
                ...
                cout << ANSI_BOLD_CYN
                     << "[TRACE XJ] Saved unfoldedTruth_pTgamma_xJ_colz for " << rKey
                     << " (still inside unfolded-2D canvas scope)"
                     << ANSI_RESET << "\n";
            }
            
            cout << ANSI_BOLD_CYN
                 << "[TRACE XJ] Exited unfolded-2D canvas scope for " << rKey
                 << ANSI_RESET << "\n";
            ...
            cout << ANSI_BOLD_CYN
                 << "[TRACE XJ] Allocated lines/per-photon containers for " << rKey
                 << " ; next step is post-unfold per-bin processing"
                 << ANSI_RESET << "\n";
