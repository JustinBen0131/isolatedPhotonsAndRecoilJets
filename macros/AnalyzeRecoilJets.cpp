...
// ===================================================================
// Consolidated event summary — one table for ALL datasets per centrality
// ===================================================================
void RunConsolidatedEventSummary()
{
    // =============================================================
    //  Section A: Active cut configurations
    // =============================================================
    cout << ANSI_BOLD_CYN
    << "\n================================================================\n"
    ...
    cout << ANSI_BOLD_YEL << "\n  [CUT CONFIGURATION]\n" << ANSI_RESET;
    cout << "  PP / non-embedded SIM cuts  (CfgTag):\n"
    << "    JetPtMin   = " << kJetPtMin   << " GeV\n"
    << "    B2BCut     = " << kB2BCut      << "\n"
    << "    VzCut      = " << kVzCut       << " cm\n"
    << "    IsoConeR   = " << kIsoConeR    << "\n"
    << "    IsoMode    = " << kIsoMode     << "\n"
    << "    -> CfgTag()           = " << CfgTag() << "\n";
    cout << "  AuAu / embedded SIM cuts  (CfgTagAA / CfgTagWithUE_AA):\n"
    << "    JetPtMin   = " << kAA_JetPtMin << " GeV\n"
    << "    B2BCut     = " << kAA_B2BCut   << "\n"
    << "    VzCut      = " << kAA_VzCut    << " cm\n"
    << "    IsoConeR   = " << kAA_IsoConeR << "\n"
    << "    IsoMode    = " << kAA_IsoMode  << "\n"
    << "    UEVariant  = " << kAA_UEVariant << "\n"
    << "    -> CfgTagAA()         = " << CfgTagAA() << "\n"
    << "    -> CfgTagWithUE_AA()  = " << CfgTagWithUE_AA() << "\n";
    
    // =============================================================
    //  Section B: Probe all relevant files
    // =============================================================
    ...
    auto FmtN = [](int n, int w = 12) -> string
    {
        if (n < 0) return string(std::max(0, w - 1), ' ') + "-";
        string s = std::to_string(n);
        return string(std::max(0, w - (int)s.size()), ' ') + s;
    };
    ...
    struct ProbeDef {
        ...
        string topDir;
        string label;
        string cfgUsed;   // which CfgTag applies
        bool   isSim;
        bool   hasCent;   // does this file have per-centrality histograms?
    };
    vector<ProbeDef> probes;
    
    // --- PP data ---
    probes.push_back({InputPP(isRun25pp), kTriggerPP,
        string("PP data") + (isRun25pp ? " (Run25)" : " (Run24)"),
        CfgTag(), false, false});
    
    // --- Non-embedded photon+jet SIM (always probe 5/10/20 individually) ---
    probes.push_back({InputSim("photonjet5"),  kDirSIM, "SIM photonJet5",  CfgTag(), true, false});
    probes.push_back({InputSim("photonjet10"), kDirSIM, "SIM photonJet10", CfgTag(), true, false});
    probes.push_back({InputSim("photonjet20"), kDirSIM, "SIM photonJet20", CfgTag(), true, false});
    
    // --- AuAu data (one row per trigger) ---
    {
        const string dataPath = InputAuAu();
        for (const auto& trig : kTriggersAuAu)
            probes.push_back({dataPath, trig, "AuAu " + trig,
                CfgTagWithUE_AA(), false, true});
    }
    ...
    for (const auto& pr : probes)
    {
        ...
        r.label    = pr.label;
        r.filePath = pr.path;
        r.cfgUsed  = pr.cfgUsed;
        r.fileOK   = false;
        r.hasCent  = pr.hasCent;
        r.nTotal   = -1;
        r.nCent.resize(centBins.size(), -1);
        
        if (gSystem->AccessPathName(pr.path.c_str()))
        {
            results.push_back(r);
            continue;
        }
        TFile* f = TFile::Open(pr.path.c_str(), "READ");
        if (!f || f->IsZombie())
        {
        ...
        TDirectory* top = f->GetDirectory(pr.topDir.c_str());
        if (!top) top = f;
        
        r.fileOK = true;
        
        if (pr.isSim)
            r.nTotal = GetN(top, "cnt_SIM");
        else
            r.nTotal = GetN(top, "cnt_" + pr.topDir);
        
        if (pr.hasCent)
        {
            for (size_t i = 0; i < centBins.size(); ++i)
                r.nCent[i] = GetN(top, "h_HT_r04" + centBins[i].suffix);
        }
    ...
    // =============================================================
    //  Section C: Print table — PP / non-embedded SIM (no centrality)
    // =============================================================
    cout << ANSI_BOLD_YEL << "\n  [PP & NON-EMBEDDED SIM]\n" << ANSI_RESET;
    cout << "  " << std::setw(20) << std::left << "Dataset"
    ...
    {
        if (r.hasCent) continue;
        cout << "  " << std::setw(20) << std::left << r.label;
        if (!r.fileOK)
            cout << ANSI_BOLD_RED << std::setw(12) << std::right << "NOT FOUND" << ANSI_RESET;
        else
            cout << FmtN(r.nTotal, 12);
        cout << "   [" << r.cfgUsed << "]  " << r.filePath << "\n";
    }
    
    // =============================================================
    //  Section D: Print table — AuAu data + embedded SIM (with centrality)
    // =============================================================
    cout << ANSI_BOLD_YEL << "\n  [AuAu DATA & EMBEDDED SIM]  (per centrality)\n" << ANSI_RESET;
    const int colW = 12;
    cout << "  " << std::setw(20) << std::left << "Dataset"
    << std::setw(colW) << std::right << "Total";
    for (const auto& cb : centBins)
        cout << std::setw(colW) << std::right << (cb.label + "%");
    cout << "\n";
    cout << "  " << string(20 + colW + (int)centBins.size() * colW, '-') << "\n";
    ...
    for (const auto& r : results)
    {
        ...
        if (!r.fileOK)
        {
            cout << ANSI_BOLD_RED << std::setw(colW) << std::right << "NOT FOUND" << ANSI_RESET;
        }
        else
        {
            cout << FmtN(r.nTotal, colW);
            for (size_t i = 0; i < centBins.size(); ++i)
                cout << FmtN(r.nCent[i], colW);
        }
        cout << "\n";
    }
    
    // Print file paths for centrality rows separately (keeps table compact)
    cout << "\n  File paths:\n";
    for (const auto& r : results)
    {
        if (!r.hasCent) continue;
        cout << "    " << std::setw(20) << std::left << r.label
        << " [" << r.cfgUsed << "]  " << r.filePath << "\n";
    }
...
void RunABCDPurityAndSidebandSubtraction(Dataset& ds, const LeakageFactors& lf)
{
    ...
    // purity plots: use physical p_{T}^{#gamma} on the x axis and explicit purity uncertainties
    {
        ...
        // purity_raw plot
        {
            TGraphErrors gRaw(kNPtBins - 1, &x[1], &yRaw[1], &ex[1], &eyRaw[1]);
            gRaw.SetLineWidth(2);
            ...
            TH1F hFrame("hPurRawFrame","",100, kPtEdges[1], kPtEdges.back());
            hFrame.SetDirectory(nullptr);
            hFrame.SetStats(0);
            ...
            if (!ds.isSim && !ds.centFolder.empty())
            {
                ...
                if (ppDirPur)
                {
                    ...
                    if (anyPP)
                    {
                        gPP = new TGraphErrors(kNPtBins - 1, &xPP[1], &yPP[1], nullptr, &eyPP[1]);
                        gPP->SetLineWidth(2);
...
void JES3_R02R04Overlays_MaybeRun(Dataset& ds, const std::string& outDir)
{
        ...
        int colorB = kRed + 1)->void
    {
        ...
        TLegend leg(0.68, 0.78, 0.92, 0.92);
        leg.SetBorderSize(0);
        leg.SetFillStyle(0);
        leg.SetTextFont(42);
        leg.SetTextSize(0.032);
        ...
        {
            const string bbLbl = (kAA_B2BCut == "7pi_8")
            ? "|#Delta#phi| > 7#pi/8" : "|#Delta#phi| > #pi/2";
            const string isoRLbl = (kAA_IsoConeR == "isoR40")
            ? "#DeltaR_{cone} < 0.4" : "#DeltaR_{cone} < 0.3";
            const string isoModeLbl = (kAA_IsoMode == "fixedIso4GeV")
            ? "E_{T}^{iso} < 4 GeV"
            : (kAA_IsoMode == "fixedIso5GeV")
            ? "E_{T}^{iso} < 5 GeV"
            : "E_{T}^{iso} sliding";
            ...
            tCuts.SetTextFont(42);
            tCuts.SetTextAlign(13);
            tCuts.SetTextSize(0.030);
            double yC = 0.86;
            const double dy = 0.045;
            tCuts.DrawLatex(0.16, yC, TString::Format("|v_{z}| < %d cm", kAA_VzCut).Data());                yC -= dy;
            tCuts.DrawLatex(0.16, yC, isoRLbl.c_str());                                                     yC -= dy;
            tCuts.DrawLatex(0.16, yC, isoModeLbl.c_str());                                                   yC -= dy;
            tCuts.DrawLatex(0.16, yC, bbLbl.c_str());                                                        yC -= dy;
            tCuts.DrawLatex(0.16, yC, TString::Format("p_{T}^{jet} > %d GeV", kAA_JetPtMin).Data());         yC -= dy;
            tCuts.DrawLatex(0.16, yC, TString::Format("UE: %s", kAA_UEVariant.c_str()).Data());
        }
...
int Run()
{
        ...
        (mode == RunMode::kSimOnly && IsEmbeddedSimSample(CurrentSimSample())))
    {
        ...
        cout << ANSI_BOLD_CYN << "\n[STEP 3c] Embedded SIM statistics diagnostic\n" << ANSI_RESET;
    ...
    if (!SSoverlayPerVAR_processONLY)
    {
        ...
        // ---------------------------------------------------------------------------
        // AuAu-only / SIM+AuAu: raw ABCD purity overlay across centrality bins (per trigger)
