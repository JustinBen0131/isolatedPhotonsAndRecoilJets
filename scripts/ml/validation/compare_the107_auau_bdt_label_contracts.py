...
ET_BINS = [(15.0, 20.0), (20.0, 25.0), (25.0, 30.0), (30.0, 35.0)]
...
def candidate_keys(
    payload: dict[str, np.ndarray], source: Path
) -> tuple[np.ndarray, dict]:
    """Build a bit-exact identity for cross-lane holdout intersections.

    ``input_file_index`` is assigned within each staged cache shard.  After
    shard reduction, the provenance-only columns can therefore collide for
    distinct candidates from different shards.  The ordered feature row is
    label independent and is preserved bit-for-bit in both model lanes, so it
    safely disambiguates those shard-local collisions without dropping rows.
    """
    columns = [
        "source_role",
        "source_sample_code",
    ...
    required(payload, columns + ["x"], source)
    arrays = [np.asarray(payload[name], dtype="int64") for name in columns]
    base_key = np.rec.fromarrays(arrays, names=",".join(columns))
    _base_unique, base_counts = np.unique(base_key, return_counts=True)
    duplicate_groups = int(np.sum(base_counts > 1))
    duplicate_extra_rows = int(np.sum(base_counts[base_counts > 1] - 1))
    max_base_multiplicity = int(base_counts.max()) if len(base_counts) else 0

    x = np.ascontiguousarray(payload["x"], dtype="float32")
    if x.ndim != 2 or len(x) != len(base_key):
        raise SystemExit(
            f"candidate identity feature matrix has unexpected shape in {source}: {x.shape}"
        )
    feature_bits = x.view(np.uint32).reshape(x.shape)
    feature_names = [f"feature_bits_{index:02d}" for index in range(x.shape[1])]
    key = np.rec.fromarrays(
        arrays + [feature_bits[:, index] for index in range(x.shape[1])],
        names=",".join(columns + feature_names),
    )
    unique_rows = len(np.unique(key))
    if unique_rows != len(key):
        raise SystemExit(
            "candidate identity remains non-unique after bit-exact feature "
            f"qualification in {source}: rows={len(key)} unique={unique_rows}"
        )
    return key, {
        "rows": int(len(key)),
        "base_key_columns": columns,
        "base_key_duplicate_groups": duplicate_groups,
        "base_key_duplicate_extra_rows": duplicate_extra_rows,
        "base_key_max_multiplicity": max_base_multiplicity,
        "qualification": "ordered_float32_feature_bits",
        "qualification_columns": feature_names,
        "qualified_key_unique_rows": int(unique_rows),
        "qualified_key_is_unique": True,
    }
...
def load_lane(
    ...
    keys, identity_audit = candidate_keys(payload, holdout_path)
    return {
        "name": name,
        "payload": payload,
        "features": features,
...
def slice_metrics(labels, scores, weights, mask) -> dict:
    local_y = labels[mask]
    local_s = scores[mask]
...
def high_score_background_metrics(lane: dict, low: float, high: float) -> dict:
    labels = lane["labels"]
    scores = lane["scores"]
    weights = lane["weights"]
    payload = lane["payload"]
    centrality = np.asarray(payload["centrality"], dtype="float64")
    cluster_et = np.asarray(payload["cluster_Et"], dtype="float64")
    source_code = np.asarray(payload["source_sample_code"], dtype="int64")
    truth_prompt = np.asarray(payload["truth_is_prompt"], dtype="int8") == 1
    cent_mask = (centrality >= low) & (centrality < high)
    local_wp = wp80_metrics(labels[cent_mask], scores[cent_mask], weights[cent_mask])
    threshold = float(local_wp["threshold"])
    background = cent_mask & (labels == 0)
    selected = background & (scores > threshold)
    selected_weight = float(np.sum(weights[selected]))
    sources = []
    for code in sorted(np.unique(source_code[background]).tolist()):
        source_background = background & (source_code == code)
        source_selected = selected & (source_code == code)
        source_weight = float(np.sum(weights[source_selected]))
        denominator = float(np.sum(weights[source_background]))
        sources.append(
            {
                "source_sample_code": int(code),
                "background_rows": int(source_background.sum()),
                "high_score_rows": int(source_selected.sum()),
                "weighted_acceptance": source_weight / denominator if denominator > 0.0 else math.nan,
                "fraction_of_high_score_background_weight": (
                    source_weight / selected_weight if selected_weight > 0.0 else math.nan
                ),
            }
        )
    finite_et = selected & np.isfinite(cluster_et)
    return {
        "centrality": [low, high],
        "wp80_threshold": threshold,
        "background_rows": int(background.sum()),
        "high_score_background_rows": int(selected.sum()),
        "weighted_background_acceptance": float(local_wp["background_acceptance"]),
        "truth_prompt_rows_in_high_score_background": int(np.sum(selected & truth_prompt)),
        "truth_prompt_fraction_in_high_score_background": (
            float(np.sum(weights[selected & truth_prompt]) / selected_weight)
            if selected_weight > 0.0
            else math.nan
        ),
        "mean_score": finite_mean(scores[selected]),
        "mean_cluster_et": finite_mean(cluster_et[finite_et]),
        "p90_cluster_et": (
            float(np.quantile(cluster_et[finite_et], 0.90)) if finite_et.any() else math.nan
        ),
        "source_composition": sources,
    }
...
def native_metrics(lane: dict) -> dict:
    ...
    cluster_et = np.asarray(lane["payload"]["cluster_Et"], dtype="float64")
    bins = []
    for low, high in CENTRALITY_BINS:
        mask = (centrality >= low) & (centrality < high)
        entry = {"centrality": [low, high]}
        entry.update(slice_metrics(labels, scores, weights, mask))
        bins.append(entry)
    et_bins = []
    for low, high in ET_BINS:
        mask = (cluster_et >= low) & (cluster_et < high)
        entry = {"cluster_et": [low, high]}
        entry.update(slice_metrics(labels, scores, weights, mask))
        et_bins.append(entry)
    return {
        "rows": int(len(labels)),
        ...
        "cluster_et_bins": et_bins,
        "high_score_background": [
            high_score_background_metrics(lane, low, high)
            for low, high in CENTRALITY_BINS
        ],
    }
...
def main() -> int:
    ...
    metrics = {
        ...
        "candidate_identity": {
            nominal["name"]: nominal["identity_audit"],
            variant["name"]: variant["identity_audit"],
        },
