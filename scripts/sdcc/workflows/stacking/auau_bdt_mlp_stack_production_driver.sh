...
RJ_REPO_BASE="$(rj_find_repo_root "$SCRIPT_DIR")"
...
STACK_CAMPAIGN_TAG="${STACK_CAMPAIGN_TAG:-bdt_mlp_stack_wp080_${STAMP}}"
OUTPUT_BASE="${OUTPUT_BASE:-$(rj_merge_root_for_campaign "$STACK_CAMPAIGN_TAG")}"
DEST_BASE="${DEST_BASE:-$(rj_recoiljets_bulk_root)/stacking/${STACK_CAMPAIGN_TAG}}"
...
case "$mode" in
  ...
  submitPair)
    ...
    log="/tmp/submit_bdt_mlp_stack_${STAMP}.log"
    {
      ...
      export RJ_SIMEMBED_DEST_BASE="${DEST_BASE}/signal"
      export RJ_SIMEMBEDINCLUSIVE_DEST_BASE="${DEST_BASE}/background"
