...
python3 - \
  ...
  "$candidate_csv" "$expected_token" <<'PY'
from pathlib import Path
import hashlib
import json
import sys
...
(
    ...
    recoil_root, candidate_csv, token,
) = sys.argv[1:]
...
call = f"Fun4All_ppg12_fixed_seed_oracle(42,{json.dumps(g4)},{json.dumps(truth)},{json.dumps(output)})"
...
call = f"Fun4All_recoiljets_fixed_seed_oracle(42,{json.dumps(combined)},{json.dumps(output)})"
Path(runner).write_text(f'''{{
  Int_t error = 0;
  gROOT->SetMacroPath((std::string({json.dumps(calo_macro_dir + ':')}) + gROOT->GetMacroPath()).c_str());
...
(
  ...
  export RJ_PPG12_PPSIM_FIXED_RANDOMSEED=42
  export RJ_PPG12_PPSIM_FIXED_PEDESTAL_SEQUENCE=1482
