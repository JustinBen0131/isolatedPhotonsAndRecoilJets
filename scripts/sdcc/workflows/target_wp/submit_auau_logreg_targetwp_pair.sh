...
yaml="${RJ_LOGREG_TARGETWP_CONFIG_YAML:-${1:-}}"
[[ -n "$yaml" ]] || { echo "[ERROR] Set RJ_LOGREG_TARGETWP_CONFIG_YAML=/path/to/generated_logreg_targetwp.yaml or pass it as argv[1]" >&2; exit 2; }
[[ -f "$yaml" ]] || { echo "[ERROR] Config YAML not found: $yaml" >&2; exit 2; }

base_name="$(basename "$yaml" .yaml)"
campaign_tag="${RJ_LOGREG_TARGETWP_CAMPAIGN_TAG:-${base_name}_$(date +%Y%m%d_%H%M%S)}"
group_size="${RJ_LOGREG_TARGETWP_GROUP_SIZE:-7}"
...
auto_merge="${RJ_LOGREG_TARGETWP_AUTO_MERGE:-0}"

log_dir="${RJ_LOGREG_TARGETWP_LOG_DIR:-${repo_root}/condor_generated_configs/${campaign_tag}}"
mkdir -p "$log_dir"
submit_log="${log_dir}/submit_${campaign_tag}.log"
exec > >(tee -a "$submit_log") 2>&1

echo "RECOILJETS_AUAU_LOGREG_TARGETWP_PAIR_SUBMIT_V1"
...
if ! grep -q "auau_tight_logreg_working_point_entries" "$yaml"; then
  echo "[ERROR] YAML has no auau_tight_logreg_working_point_entries; refusing logreg target-WP submission." >&2
  exit 3
fi
if grep -Eq '^[[:space:]]*auau_tight_logreg_working_point_entries[[:space:]]*:[[:space:]]*\[[[:space:]]*\]' "$yaml"; then
  echo "[ERROR] YAML has an empty auau_tight_logreg_working_point_entries list." >&2
  exit 3
fi
if ! grep -q "auauTightLogReg" "$yaml"; then
  echo "[ERROR] YAML does not appear to include the logreg photon-ID row." >&2
  exit 4
fi
...
signal_bulk="$(rj_bulk_signal_root_for_campaign target_wp "$campaign_tag")"
background_bulk="$(rj_bulk_background_root_for_campaign target_wp "$campaign_tag")"
