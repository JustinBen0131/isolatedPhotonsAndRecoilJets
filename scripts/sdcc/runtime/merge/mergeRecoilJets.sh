...
if [[ -f "${SCRIPT_DIR}/../io/recoiljets_io_paths.sh" ]]; then
  source "${SCRIPT_DIR}/../io/recoiljets_io_paths.sh"
...
# ---------- Output base ----------
# New merges default to the Stage-7 canonical checkout path. Historical
# output/<dataset> and output_<campaign> roots remain readable via explicit
# MERGE_OUT_BASE_OVERRIDE for old campaigns.
if declare -F rj_merge_root_for_campaign >/dev/null 2>&1; then
  OUT_BASE="${MERGE_OUT_BASE_OVERRIDE:-$(rj_merge_root_for_campaign "${RJ_MERGE_CAMPAIGN_TAG:-manual}")}"
else
  OUT_BASE="${MERGE_OUT_BASE_OVERRIDE:-/sphenix/u/patsfan753/scratch/thesisAnalysis/runs/recoiljets/current/${RJ_MERGE_CAMPAIGN_TAG:-manual}}"
fi

# Per-dataset output dirs end up as:
#   runs/recoiljets/current/<campaign>/pp
