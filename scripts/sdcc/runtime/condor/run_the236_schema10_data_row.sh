...
PY
  ...
  [[ "$gate_source_pairs" =~ ^[1-9][0-9]*$ ]] || die "AuAu event-gate source-pair count differs"
...
python3 - "$time_raw" "$measurement_candidate" "$system" "$row_id" "$ClusterId" "$ProcId" "$request_memory_mb" "$candidate_path" "$output_path" "$manifest_sha" "$chunk_sha" "$expected_processed" "$root_contract" "$event_limit" "$gate_candidate_path" "$gate_output_path" "$gate_receipt_candidate" "$gate_receipt_path" <<'PY'
...
if event_limit>0:
    ...
    processed_count_contract="EXACT_BOUNDED_V1"
elif shortfall==0:
