...
def main() -> None:
    ...
    rows = [
        ("BDT\nptFine cent7", "BDT ptFine cent7", np.asarray(frame["bdt_score"], dtype="float64")),
        ("MLP\nstack input", "MLP stack input", np.asarray(frame["mlp_score"], dtype="float64")),
        ("BDT+MLP stack\nexact artifact", "BDT+MLP stack exact artifact", stack_score),
    ]
    ...
    fig, axes = plt.subplots(3, len(cent_edges) - 1, figsize=(14.6, 8.5), sharex=True)
    summary: list[dict] = []
    for irow, (row_label, model_label, score) in enumerate(rows):
        for icol, (clo, chi) in enumerate(zip(cent_edges[:-1], cent_edges[1:], strict=True)):
            ...
            density_hist(ax, sig, bkg, args.bins, "#1f77b4", "#b23a48")
            ax.text(0.045, 0.86, f"AUC = {auc:.3f}", transform=ax.transAxes, ha="left", va="top", fontsize=10.5)
            if irow == 0:
    ...
    fig.text(0.50, 0.975, r"Signal/background score separation, $15 < E_T < 35$ GeV", ha="center", va="top", fontsize=17, fontweight="bold")
