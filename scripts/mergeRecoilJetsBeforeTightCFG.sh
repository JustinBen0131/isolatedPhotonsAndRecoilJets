  ...
  for cfg_tag in "${SIM_CFG_TAGS[@]}"; do
    ...
    for samp in "${samples[@]}"; do
      ...
      if [[ "$SIM_ACTION" == "firstRound" ]]; then
        ...
        mapfile -t SIM_INPUTS < <(find "$SIM_INPUT_DIR" -maxdepth 1 -type f -name "*.root" -not -name "*_LOCAL_*" -not -name "*_condorTest_*" | sort -V || true)
        if (( ${#SIM_INPUTS[@]} == 0 )); then
          warn "No *.root files found in: $SIM_INPUT_DIR after excluding *LOCAL* and *condorTest* (skipping)"
          continue
        fi
