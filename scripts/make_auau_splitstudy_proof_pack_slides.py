...
# Read-only SDCC registry check on 2026-05-27. These are internal holdout AUCs
# recorded by the BDT trainer; train-side AUC/logloss were not recorded.
INTERNAL_HOLDOUT = {
...
def save_summary(outdir: Path, rows: list[dict[str, object]], compact: dict[str, object]) -> None:
    ...
    summary = {
        ...
        "campaign": "ppg12_weighted_centinput_splitstudy_20260522_1450",
        "main_conclusion": (
            "Global validation and internal holdout AUC decrease with less training data. "
            "The 0-20% AUC rise is small and is not mirrored by monotonic WP80 fake-rate improvement."
        ),
...
def make_main_slide(outdir: Path, rows: list[dict[str, object]], compact: dict[str, object]) -> Path:
    ...
    ax_global.set_ylim(0.80, 0.86)
    ax_global.set_ylabel("global full-stat AUC")
    ax_global.set_title("Global behavior follows intuition", weight="bold", fontsize=14)
    ax_global.grid(True, color=GRID)
    for i, y in enumerate(global_auc):
        ax_global.text(i, y + 0.003, f"{y:.3f}", ha="center", fontsize=11, weight="bold")
    ...
    ax_delta.set_title("The paradox is tiny", weight="bold", fontsize=14)
    ...
    add_small_box(
        ...
        [
            "1. More training data wins globally.",
            "2. 0-20% moves by only +0.0017 AUC.",
            "3. WP80 fake rate does not improve cleanly.",
            "4. Therefore AUC alone is not a 5x-simulation argument.",
        ],
        "#ECFDF5",
    )
    ...
    add_small_box(
        ...
        "Current verdict",
        [
            "Small 0-20 rank-order effect.",
            "Not proof that less training data is better.",
    ...
    add_verdict_box(
        ...
        "Bottom line",
        "The scary-looking trend is local and small. The production-relevant question is not raw AUC alone, "
...
def make_overfitting_slide(outdir: Path, compact: dict[str, object]) -> Path:
    ...
    add_title(
        ...
        "The current evidence argues against the simple claim that smaller training samples are better, but train-side metrics are missing.",
    )
    ...
    x = np.arange(len(SPLITS))
    internal_auc = [INTERNAL_HOLDOUT[s]["auc"] for s in SPLITS]
    global_auc = [next(r for r in compact["rows"] if r["label"] == s)["global_auc"] for s in SPLITS]
    train_rows = np.asarray([INTERNAL_HOLDOUT[s]["train_rows"] / 1e6 for s in SPLITS])

    ax_auc.plot(x, internal_auc, marker="o", lw=2.7, color=ORANGE, label="internal holdout AUC")
    ax_auc.plot(x, global_auc, marker="s", lw=2.7, color=BLUE, label="full-stat validation AUC")
    ax_auc.set_xticks(x, SPLITS)
    ax_auc.set_ylim(0.79, 0.86)
    ax_auc.set_ylabel("AUC")
    ax_auc.set_title("Held-out AUC also drops with less training", weight="bold", fontsize=14)
    ...
    validation_gap = np.asarray(global_auc) - np.asarray(internal_auc)
    ax_gap.bar(x, validation_gap * 1000.0, color=CYAN, width=0.62)
    ax_gap.axhline(0, color=INK, lw=1)
    ax_gap.set_ylim(0, 23)
    ax_gap.set_xticks(x, SPLITS)
    ax_gap.set_ylabel("full-stat minus internal holdout AUC (x1e-3)")
    ax_gap.set_title("No runaway validation collapse is visible", weight="bold", fontsize=14)
    ax_gap.grid(True, axis="y", color=GRID)
    for i, y in enumerate(validation_gap * 1000.0):
        ax_gap.text(i, y - 0.9, f"{y:+.1f}", ha="center", va="top", fontsize=11, weight="bold", color="white")

    ax_matrix.axis("off")
    matrix_rows = [
        ["internal holdout AUC", "available", "drops with less training"],
        ["full-stat validation AUC", "available", "drops globally"],
        ["0-20% validation AUC", "available", "tiny opposite-sign local shift"],
        ["train AUC", "missing", "needed for overfit claim"],
        ["train and validation logloss", "missing", "needed for calibration/overfit claim"],
    ...
    add_small_box(
        ...
        "Expert read",
        [
            "Overfitting is not established by slide 23.",
            "The expected smoking gun would be high train AUC with lower validation AUC.",
            "Current BDT output does not record train AUC/logloss.",
            "Patch diagnostics before using this as a resource argument.",
        ],
        "#F3E8FF",
    )
    ...
    add_verdict_box(
        ...
        (0.06, 0.055, 0.90, 0.105),
        "Bottom line",
        "The measured holdout and full-stat AUCs do not show smaller training samples winning globally. "
        "The unresolved overfitting question is an instrumentation gap, not a confirmed pathology.",
    )
...
def make_autopsy_slide(outdir: Path, rows: list[dict[str, object]], compact: dict[str, object]) -> Path:
    ...
    add_title(
        fig,
        "0-20% autopsy: where does the tiny AUC rise come from?",
        "Compact histogram triage says the shift is a small rank-order reshuffling, not yet a working-point improvement.",
    )
    ...
    add_small_box(
        ...
        "Autopsy verdict",
        [
            "The +0.0017 AUC rise is not a large separation change.",
            "WP80 background survival is essentially unchanged.",
            "The next decisive test is paired per-event resampling.",
            "If a single ET bin or tail drives it, this is composition/statistics, not a model-quality reversal.",
        ],
        "#ECFDF5",
    )
    ...
    add_verdict_box(
        ...
        "Do not use it to argue against more training data until paired score-cache and ET-bin decomposition are done.",
    )
