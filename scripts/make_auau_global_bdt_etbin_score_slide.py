...
def draw_slide(csv_path: Path, metadata_path: Path, out_path: Path) -> None:
    ...
    fig.text(0.794, 0.925, "PYTHIA8 Au+Au embedded validation", fontsize=10.8, ha="left", va="top", color=INK)

    add_card(
        fig,
        [0.045, 0.828, 0.285, 0.060],
        "Model shown",
        r"global $\bf{centInput\_pt1535}$ BDT",
        face=PINK_TINT,
        edge=PINK_EDGE,
    )
    add_card(
        ...
        "Inputs",
        r"baseV3E + centrality + $w_{\eta,33}/w_{\phi,33}$",
        face="#F8FAFC",
        edge="#CBD5E1",
    )
    add_card(
        ...
        "Training prior control",
        r"PPG12-style $E_T$ and $\eta$ weights",
        face=BLUE_TINT,
        edge=BLUE_EDGE,
        title_color="#1E3A8A",
        body_color="#1E3A8A",
    ...
    legend_ax = fig.add_axes([0.872, 0.828, 0.122, 0.060])
    legend_ax.axis("off")
    legend_ax.set_xlim(0, 1)
    legend_ax.set_ylim(0, 1)
    legend_ax.plot([0.02, 0.30], [0.68, 0.68], color=SIGNAL, lw=4.0, solid_capstyle="butt")
    legend_ax.text(0.36, 0.68, "Signal", fontsize=11.0, color=INK, ha="left", va="center", clip_on=False)
    legend_ax.plot([0.02, 0.30], [0.30, 0.30], color=BACKGROUND, lw=4.0, solid_capstyle="butt")
    legend_ax.text(0.36, 0.30, "Background", fontsize=11.0, color=INK, ha="left", va="center", clip_on=False)
    ...
    band.text(
        0.130,
        0.24,
        rf"Background shifts toward signal-like scores around 21-25 GeV; full-stat held-out test has {total_rows:,} scored rows, $15 \leq E_T < 35$ GeV, 0-80% centrality.",
        fontsize=11.2,
        color=MUTED,
        ha="left",
        va="center",
