...
def render_overlay(rows: list[dict[str, float]], reference: dict[str, list[dict[str, float]]], output: Path) -> None:
    ...
    label.DrawLatex(0.15, 0.78, "PPG12 SDCC vs THE97   data: PPG12_scaledtrigger30")
    ...
    legend.AddEntry(ppg_corr, "PPG12 leakage corrected", "p"); legend.AddEntry(cur_corr, "THE97 leakage corrected", "p")
    legend.AddEntry(ppg_raw, "PPG12 raw", "p"); legend.AddEntry(cur_raw, "THE97 raw", "p"); legend.Draw()
    top.RedrawAxis()
    bottom.cd()
