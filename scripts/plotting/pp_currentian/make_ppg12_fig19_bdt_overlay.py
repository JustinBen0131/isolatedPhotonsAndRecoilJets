...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--sample-weight-mode",
        choices=["branch", "ppg12-shower-shape"],
        default="branch",
        help=(
            "branch uses only --weight-branch and --sample-weight-scale. "
            "ppg12-shower-shape also applies the PPG12 ShowerShapeCheck.C "
            "per-sample cross-section factors before unit normalization."
        ),
    )
    ap.add_argument(
        "--require-ppg12-truth-window",
        action="store_true",
        help=(
            "For inclusive MC, require a ppg12_truth_window_pass_r04 branch and "
            "apply it. This is needed for a strict PPG12 Fig. 13/19 histogram "
            "contract; current legacy BDT trees do not carry this branch."
        ),
    )
...
PPG12_SHOWER_SHAPE_WEIGHTS = {
    # CrossSectionWeights.h normalizes photon samples to photon20 and jet
    # samples to jet50 before the histograms are unit-normalized.
    "photon5": 146359.3 / 130.4461,
    "photon10": 6944.675 / 130.4461,
    "photon20": 1.0,
    "jet5": 1.3878e8 / 7.3113,
    "jet8": 1.15e7 / 7.3113,
    "jet12": 1.4903e6 / 7.3113,
    "jet20": 6.2623e4 / 7.3113,
    "jet30": 2.5298e3 / 7.3113,
    "jet40": 1.3553e2 / 7.3113,
}


def sample_key_for_path(path: Path) -> str | None:
    text = str(path).lower()
    # Check photon samples first because "photonjet10" contains "jet10".
    sample_patterns = [
        ("photon5", ("photonjet5", "photon5")),
        ("photon10", ("photonjet10", "photon10")),
        ("photon20", ("photonjet20", "photon20")),
        ("jet5", ("run28_jet5", "jet5")),
        ("jet8", ("run28_jet8", "jet8")),
        ("jet12", ("run28_jet12", "jet12")),
        ("jet20", ("run28_jet20", "jet20")),
        ("jet30", ("run28_jet30", "jet30")),
        ("jet40", ("run28_jet40", "jet40")),
    ]
    for sample, patterns in sample_patterns:
        if any(pattern in text for pattern in patterns):
            return sample
    return None


def ppg12_shower_shape_weight_for_path(path: Path) -> float:
    sample = sample_key_for_path(path)
    if sample is None:
        return 1.0
    return float(PPG12_SHOWER_SHAPE_WEIGHTS.get(sample, 1.0))
    ...
    for path in paths[: args.max_files or None]:
        ...
        try:
            with uproot.open(path) as root_file:
                ...
                if label == "inclusive" and args.require_ppg12_truth_window:
                    if "ppg12_truth_window_pass_r04" not in tree.keys():
                        raise SystemExit(
                            f"{path} is missing ppg12_truth_window_pass_r04. "
                            "This tree cannot reproduce the strict PPG12 inclusive "
                            "truth-window contract; rerun extraction with the "
                            "PPG12 window branches enabled."
                        )
                have = [col for col in columns if col in tree.keys()]
                if label == "inclusive" and args.require_ppg12_truth_window:
                    have.append("ppg12_truth_window_pass_r04")
                ...
                for arrays in tree.iterate(have, library="np", step_size=args.step_size):
                    ...
                    if label == "inclusive" and args.require_ppg12_truth_window:
                        truth_window = arrays["ppg12_truth_window_pass_r04"].astype("float64")
                        mask &= np.isfinite(truth_window) & (truth_window > 0.5)
                    ...
                    scale = weight_scale_for_path(path, sample_scales)
                    if args.sample_weight_mode == "ppg12-shower-shape":
                        scale *= ppg12_shower_shape_weight_for_path(path)
                        counts["ppg12_weighted_chunks"] += 1
                    weight = weight_all[mask] * scale
