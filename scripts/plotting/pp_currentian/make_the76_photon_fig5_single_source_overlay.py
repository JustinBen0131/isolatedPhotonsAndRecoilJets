#!/usr/bin/env python3
"""Regenerate THE-76 photon Fig.5 overlay with one RecoilJets source component.

This diagnostic intentionally does not use the four-component July 1 aggregate
(`0mrad/1p5mrad x single/double`). It aggregates only one simple photon source
component, stitches photon5/10/20 with the PPG12 ownership windows, and renders
the existing ROOT-style Fig.5 overlay plus the two-panel ratio summary.
"""
...
CAMPAIGN_FIX = "the76_ppg12_fig24_photonjet_fix_20260702_014217"
BASE_FULL = REPO / "dataOutput/ppg12Parity" / CAMPAIGN_FULL
BASE_FIX = REPO / "dataOutput/ppg12Parity" / CAMPAIGN_FIX
OUT_DIR = BASE_FIX / "strict_stitched_photonjet_single_source"
...
POINTS_CSV = OUT_DIR / "photon_data_over_fit_sdcc_vs_current_overlay_single_source_points.csv"
POINTS_MANIFEST = OUT_DIR / "single_source_points_manifest.json"

PERIOD = "0mrad"
COMPONENT = "single_photon"
REMOTE_BASE = f"/sphenix/tg/tg01/bulk/jbennett/thesisAna/sim/{CAMPAIGN_FULL}"
SETTING = "jetMinPtScan_dphiScan_vz60_isoR40_isSliding_preselectionNewPPG12_tightNewPPG12_nonTightNewPPG12"
HIST = "SIM/h_ppPhotonStitch_ppg12Fig5_maxPhotonPt_all"
METADATA = "SIM/h_ppPhotonStitch_ppg12TruthSpectrum_metadata"
...
hist_name = {HIST!r}
meta_name = {METADATA!r}
samples = {json.dumps(SAMPLES)}
out = {{}}
for sample, spec in samples.items():
    d = base / period / component / setting / spec["remote_dir"]
    ...
    out[sample] = {{
        ...
        "source_dir": str(d),
        "files": len(files),
        "events_processed_metadata": events,
        "missing": missing,
        "zombie": zombie,
        "edges": edges,
        "values": values,
...
def write_points(aggregate: dict) -> tuple[Path, Path]:
    ...
    for sample, spec in SAMPLES.items():
        ...
        events = float(rec["events_processed_metadata"])
        ...
        for i, raw in enumerate(values):
            ...
            if not srow:
                continue
            cur = raw * xsec / events if events > 0 else math.nan
            cur_err = errors[i] * xsec / events if events > 0 else math.nan
            ...
            rows.append(
                {
                    ...
                    "current_value_mode": "single_source_raw_times_xsec_over_component_metadata_no_binwidth",
                    "current_source_files": int(rec["files"]),
                    "current_events_processed_metadata": events,
                    "comparison_scope": "single-source 0mrad/single_photon photon5+10+20 stitch; no period/SI-DI aggregate; no global scale",
                }
            )
    csv_path = POINTS_CSV
    fields = [
        ...
        "current_over_fit_error", "current_over_sdcc", "current_value_mode",
        "current_source_files", "current_events_processed_metadata", "comparison_scope",
    ]
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows(rows)
    manifest_path = POINTS_MANIFEST
    manifest = {
        "status": "ok",
        ...
        "component": COMPONENT,
        "histogram": HIST,
        "metadata_histogram": METADATA,
        "samples": aggregate,
        "normalization": "current_value = raw histogram bin content * xsec / component events_processed_metadata; no bin-width division and no global scale",
        "important_caveat": "This is a single-source component diagnostic that removes the four-component period/SI-DI aggregate. It is not the default full period/SI-DI physics aggregate.",
    }
...
def run_root_plot(points_csv: Path) -> tuple[Path, Path]:
    png = OUT_DIR / "photon_data_over_fit_sdcc_vs_current_overlay_root_ppg12_style_single_source.png"
    manifest = OUT_DIR / "photon_data_over_fit_sdcc_vs_current_overlay_root_ppg12_style_single_source_manifest.json"
    scope = "Single-source Fig.5 diagnostic: PPG12 SDCC open circles vs RecoilJets 0mrad/single_photon photon5+10+20 stitch; no period/SI-DI aggregate and no global scale"
    call = f'{ROOT_MACRO}("{points_csv}","{png}","{manifest}","{scope}")'
    # Batch mode avoids the macOS Cocoa canvas path in desktop sessions.
    subprocess.run(
...
def write_ratio_summary(points_csv: Path) -> tuple[Path, Path, Path]:
    out_png = OUT_DIR / "ppg12_over_current_stitched_spectra_two_panel_summary_single_source_photon.png"
    out_csv = OUT_DIR / "ppg12_over_current_stitched_spectra_two_panel_summary_single_source_photon_points.csv"
    out_manifest = OUT_DIR / "ppg12_over_current_stitched_spectra_two_panel_summary_single_source_photon_manifest.json"
    rows: list[dict[str, object]] = []
            ...
            rows.append(
                {
                    ...
                    "current_definition": "single-source 0mrad/single_photon photon+jet stitch, no period/SI-DI aggregate",
                }
            )
    ...
    panels = [
        ("photon+jet", ["photon5", "photon10", "photon20"], (10, 40), (0.94, 1.06), "photon+jet stitched spectra, single-source diagnostic"),
    ...
    out_manifest.write_text(json.dumps({
        ...
        "photon_current_definition": "single-source 0mrad/single_photon RecoilJets points; no period/SI-DI aggregate",
...
def main() -> None:
    if POINTS_CSV.exists() and POINTS_MANIFEST.exists():
        points_csv, points_manifest = POINTS_CSV, POINTS_MANIFEST
    else:
