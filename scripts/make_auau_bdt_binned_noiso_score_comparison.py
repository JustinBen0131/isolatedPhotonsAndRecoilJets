#!/usr/bin/env python3
"""Make a 2x3 Global-BDT vs binned-BDT score-separation comparison."""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--global-validation", type=Path, required=True)
    ap.add_argument("--binned-validation", type=Path, required=True)
    ap.add_argument("--binned-product", default="globalEtCent1535_bdt_noIso_ptCent7")
...
def load_binned_hist_from_csv(path: Path, product: str) -> dict[str, dict[str, np.ndarray | float | int]]:
    ...
    if not rows:
        raise SystemExit(f"No rows found for product {product} in {path}")
    out = {}
...
def load_binned_hist_from_score_caches(
    validation_dir: Path, product: str, edges: np.ndarray
...
def load_binned_hist(validation_dir: Path, product: str, edges: np.ndarray) -> dict[str, dict[str, np.ndarray | float | int]]:
    csv_path = validation_dir / "coarse_centrality_score_histograms_binned_noIso.csv"
    if csv_path.exists():
        return load_binned_hist_from_csv(csv_path, product)
    return load_binned_hist_from_score_caches(validation_dir, product, edges)
...
def main() -> None:
    ...
    global_hist = json.loads((args.global_validation / "score_histograms.json").read_text())
    global_metrics = json.loads((args.global_validation / "validation_metrics.json").read_text())
    edges = np.array(global_hist["bin_edges"], dtype=float)
    binned_hist = load_binned_hist(args.binned_validation, args.binned_product, edges)
    global_product = global_hist["products"]["globalEtCent1535_bdt_noIso"]["by_centrality"]
    global_auc = global_metrics["products"]["globalEtCent1535_bdt_noIso"]["auc_by_centrality"]
    ...
    bkg_color = "#d62728"
    row_defs = [
        ("Global BDT", global_product),
        ("Binned BDT", binned_hist),
    ]

    for irow, (model_label, payload) in enumerate(row_defs):
            ...
            summary.append(
                {
                    ...
                    "binned_product": args.binned_product if irow == 1 else "globalEtCent1535_bdt_noIso",
                }
            )
    ...
    fig.text(
        0.57,
        0.975,
        "Signal/background score separation: global vs binned BDT",
        ha="center",
        va="top",
        fontsize=18.5,
        fontweight="bold",
    ...
    fig.text(
        0.57,
        0.020,
        "Both rows use the baseline feature set; binned row uses independent E_T x centrality models.",
        ha="center",
        va="bottom",
        fontsize=10.5,
        color="0.35",
    ...
    suffix = args.binned_product.replace("globalEtCent1535_bdt_noIso_", "")
    png = args.outdir / f"global_noiso_bdt_vs_binned_noiso_bdt_{suffix}_score_separation_by_centrality.png"
    csv_path = args.outdir / f"global_noiso_bdt_vs_binned_noiso_bdt_{suffix}_score_separation_by_centrality.csv"
