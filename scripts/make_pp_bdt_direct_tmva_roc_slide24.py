...
def main() -> None:
    ...
    ax = fig.add_axes([0.070, 0.150, 0.540, 0.685])
    ...
    ax.text(0.232, 0.945, "Internal", transform=ax.transAxes, ha="left", va="top", fontsize=18, color=INK)
    ax.text(
        0.055,
        0.865,
        ...
        color=INK,
        linespacing=1.15,
        bbox=dict(boxstyle="round,pad=0.30", fc="white", ec="#E5E7EB", alpha=0.92),
    )

    rhs_x = 0.675
    rounded_box(fig, 0.660, 0.700, 0.300, 0.145, SOFT_BLUE, "#BFDBFE")
    text_block(fig, rhs_x, 0.820, "What changed", size=18, color=BLUE, weight="bold")
    wrapped_block(
        fig,
        rhs_x,
        0.780,
        "This replaces the old ROC built from projected ROOT histograms.\n\n"
        "The red curve is the PPG12 TMVA model evaluated directly on the same candidate rows.",
        width=56,
        size=11.3,
        linespacing=1.22,
    )

    rounded_box(fig, 0.660, 0.485, 0.300, 0.165, SOFT_GREEN, "#A7F3D0")
    text_block(fig, rhs_x, 0.620, "Direct-model result", size=18, color=GREEN, weight="bold")
    wrapped_block(
        fig,
        rhs_x,
        0.580,
        ...
        "Both curves are in the expected high-AUC baseV3E range.",
        width=56,
        size=12.2,
        linespacing=1.22,
    )

    rounded_box(fig, 0.660, 0.245, 0.300, 0.185, "white", PANEL_EDGE)
    text_block(fig, rhs_x, 0.395, "Same-row audit inputs", size=17, color=INK, weight="bold")
    ...
    rounded_box(fig, 0.660, 0.075, 0.300, 0.115, SOFT_AMBER, "#FDBA74")
    fig.text(rhs_x, 0.155, "Takeaway:", ha="left", va="top", fontsize=12.3, fontweight="bold", color=INK)
    fig.text(
        0.745,
        0.150,
        "the old low PPG12 AUC came from the\n"
        "histogram-projection comparison, not the\n"
        "direct baseV3E TMVA model application.",
        ha="left",
        va="top",
        fontsize=11.3,
        color=INK,
