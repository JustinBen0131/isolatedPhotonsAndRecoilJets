...
def _event_sources(root: uproot.ReadOnlyDirectory) -> dict[tuple[int, int], int]:
    arrays = root["events"].arrays(
        ["source_file_index", "event_id_hi", "event_id_lo"], library="np"
    )
    ...
    for source, high, low in zip(
        ...
        result[identity] = int(source)
    return result


def _validate_parent_events(
    root: uproot.ReadOnlyDirectory,
    event_sources: dict[tuple[int, int], int],
) -> None:
    for tree_name in TREE_ORDER[2:]:
        ...
        for row, (source, high, low) in enumerate(
            zip(arrays["source_file_index"], arrays["event_id_hi"], arrays["event_id_lo"])
        ):
            event = _key(high, low)
            if event not in event_sources:
                raise ValueError(f"{tree_name} row {row} has no parent event")
            if event_sources[event] != int(source):
                raise ValueError(f"{tree_name} row {row} has the wrong source_file_index")
...
def _validate_truth_links(root: uproot.ReadOnlyDirectory) -> None:
    ...
    for row in range(len(links["link_id_hi"])):
        ...
        for side in ("reco", "truth"):
            ...
            if target_type == 0:
                if target_index != -1:
                    raise ValueError(f"absent {side} target has an index at link row {row}")
                continue
            if target_type not in target_maps[side]:
                raise ValueError(f"unsupported {side} target type at link row {row}")
            identity = _key(links[f"{side}_id_hi"][row], links[f"{side}_id_lo"][row])
            observed = target_maps[side][target_type].get(identity)
    ...
    *,
    ...
    for path in paths:
        ...
        with uproot.open(path) as root:
            ...
            event_sources = _event_sources(root)
            _validate_parent_events(root, event_sources)
