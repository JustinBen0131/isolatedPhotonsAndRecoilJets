...
MATPLOTLIB_EXPERIMENT_LABEL = r"$\mathit{\mathbf{sPHENIX}}$ Internal"
STYLE_ID = "sphenix_internal_provenance_compiled_v1"
...
def _selection_from_payload(payload: dict[str, Any]) -> RecoilSelection:
    ...
    return RecoilSelection(**selection)
...
def _annotation_lines(program: SelectionProgram, dialect: str) -> list[str]:
    ...
    if scope == "event_leading":
        ...
        if facts["photon.id_state"] == "non_tight":
            definition = {
                "bounded": "bounded score band",
                "complement": "tight-score complement",
            }[str(facts["photon.non_tight_definition"])]
            identity += f" ({definition})"
        isolation = "isolated" if facts["photon.isolation_state"] == "isolated" else "non-isolated"
        iso_radius = _number(float(facts["photon.isolation_radius"]))
        model = str(facts["photon.model_view"])
        bdt_operator = predicates["photon.bdt_class"]["operator"]
        bdt_definition = {
            "gt_field": "score > calibrated tight threshold",
            "open_between_fields": "calibrated bounded score band",
            "not_gt_field": "tight-threshold complement",
        }[bdt_operator]
        iso_operator = predicates["photon.isolation_class"]["operator"]
        iso_symbol = "<" if iso_operator == "lt_field" else ">"
        lines.append(
            f"Event-leading {model} {identity} photon (ABCD {region}; {bdt_definition})"
        )
        lines.append(
            (
                f"E_{{T}}^{{iso}}(R = {iso_radius}) {iso_symbol} candidate isolation threshold"
                if dialect == "root"
                else rf"$E_T^{{\mathrm{{iso}}}}(R={iso_radius}) {iso_symbol} T_{{\mathrm{{iso}}}}$"
            )
        )
...
def compile_plot_contract(
    *,
    ...
    contract: dict[str, Any] = {
        ...
        "visible_semantic_keys": sorted(
            [predicate.key for predicate in program.predicates]
            + [fact.key for fact in program.facts]
        ),
    }
...
def verify_plot_contract(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_keys = {
        ...
        "annotations", "visible_semantic_keys", "contract_sha256",
    }
