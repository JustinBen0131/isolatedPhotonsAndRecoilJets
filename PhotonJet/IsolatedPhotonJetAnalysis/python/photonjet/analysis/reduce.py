...
class RecoilSelection:
    ...
    def leader_branch(self) -> str:
        branch = compile_recoil_selection(self).leader_branch
        if branch is None:
            raise ValueError("inclusive selection has no event-leading branch")
        return branch
...
def _event_arrays(path: Path, selection: RecoilSelection) -> ak.Array:
    with uproot.open(path) as root:
        if "eventTree" not in root:
            raise ValueError(f"{path} does not contain PhotonJetTrees_v1/eventTree")
        tree = root["eventTree"]
        required = [*EVENT_BRANCHES, selection.leader_branch]
...
def _selected_rows(
    ...
    for raw_path in input_paths:
        ...
        for index in range(len(arrays)):
            ...
            recorded_leader = int(arrays[selection.leader_branch][index])
            if leader != recorded_leader:
                raise ValueError(
                    f"eventTree {selection.leader_branch}={recorded_leader} differs from "
...
def recoil_histogram(
    ...
    rows = list(_selected_rows(paths, selection))
