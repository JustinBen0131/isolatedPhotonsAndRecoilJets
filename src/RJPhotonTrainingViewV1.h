...
class Runtime
{
 public:
                  ...
                  std::string* error = nullptr)
  {
    ...
    if (source.id.isNull() || source.id != sourceOccurrenceIdentity(source) ||
                       ...
                       std::string* error = nullptr)
  {
    ...
    if (labelIt == m_labels.end()) return true;  // Selection-neutral replay-only candidate.
    if (m_consumed.count(key) != 0)
      return fail(error, "duplicate labeled training candidate");
    ...
    if (candidate.run != m_source.run ||
        candidate.event_id != eventIdentity(m_source,candidate.run,candidate.event_sequence))
      return fail(error, "training candidate event stable identity input mismatch");
    const Identity128 expectedCandidateId = makeIdentity(
        candidate.event_id.hex() + "|candidate|" +
    ...
    for (const auto& view : views)
    {
      ...
      row.id = makeIdentity(candidate.candidate_id.hex() + "|training-view|" + view.definition_id.hex());
  ...
  {
    if (!ready(error)) return false;
    if (!m_labels.empty() || !m_pendingRows.empty())
