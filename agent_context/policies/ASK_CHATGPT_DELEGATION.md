...
Exception: when Justin explicitly asks for the bounded dual-Pro overnight
research lane, Codex may stage two separate `pro_extended` prompt packs under
`agent_context/local/chatgpt_research/`, submit them in two fresh ChatGPT
conversations through Computer Use, and run a single temporary local heartbeat
until both copied responses are present. This exception is not a general Pro
polling rule. It is limited to the paired physics-thesis and OS-infrastructure
research prompts generated by:

```bash
python3 scripts/os/research/codex_chatgpt_research_pack.py init-dual-pro-overnight
```

The OS prompt must not be sent until the intended repo state is committed and
pushed to GitHub, because the prompt asks ChatGPT to inspect the current repo.
The copied responses remain external critique/source leads, not evidence,
approval, or task state. The OS response may drive at most one surgical
OS-only change after local verification, explicit rollback, and doctor
validation; it must not mutate science outputs, SDCC, Condor, Drive/Slides,
Gmail, Linear, secrets, or physics/task status.
...
- for the explicit dual-Pro overnight lane only, submit the two staged
  `pro_extended` prompts in separate fresh chats, copy finished responses into
  their local packs, and update the temporary local heartbeat until complete;
